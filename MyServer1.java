...
public class MyServer1 extends JFrame implements ActionListener,Runnable
{
	JFrame f;
	JButton b,b1,b2,b3,b10;
	TextField tf,tf1,tf2,tf3,tf11,tf22,tf33;
	
	JCheckBox ch,ch1,ch2,ch3,ch4;
	JComboBox comboBox1,comboBox2,comboBox3;
	String s[]={"3DES Cipher"};
	...
	{	
		...
		JPanel panel=new JPanel();		//THIS PANEL CONTAINS JCOMBOBOX
		JPanel panel1=new JPanel();		//THIS PANEL CONTAINS TEXTAREA FOR KEYS. 	
		...
		b3=new JButton("Send");
		b10=new JButton("NOT CONNECTED");

		comboBox1 = new JComboBox(s);	//DROP DOWN MENU1
        comboBox2 = new JComboBox(s);	//DROP DOWN MENU2
        comboBox3 = new JComboBox(s);	//DROP DOWN MENU3

        tf11=new TextField();		//TEXTFIELD TO SHOW SELECTED COMBOBOX
		tf22=new TextField();		//TEXTFIELD TO SHOW SELECTED COMBOBOX
		tf33=new TextField();		//TEXTFIELD TO SHOW SELECTED COMBOBOX

		comboBox1.setSelectedItem(s[r]);
		comboBox2.setSelectedItem(s[r1]);
		comboBox3.setSelectedItem(s[r2]);

		panel.add(comboBox1);				//ADDING COMBOBOX INSIDE PANEL
		panel.add(comboBox2);
		panel.add(comboBox3);

		panel1.add(tf11);					//ADDING TEXFIELD INSIDE PANEL1
		panel1.add(tf22);
		panel1.add(tf33);
		panel1.setLayout(new GridLayout());

		tf=new TextField();			//TEXTFIELD TO SHOW SELECTED FILE.
		tf1=new TextField();		//TEXTFIELD TO SHOW SELECTED COMBOBOX
		tf2=new TextField();		//TEXTFIELD TO SHOW SELECTED COMBOBOX
		tf3=new TextField();		//TEXTFIELD TO SHOW SELECTED COMBOBOX
		
		b.addActionListener(this);	//ACTION PERFORMED ON BUTTON
		...
		b.setBackground(Color.black);		//TO CHANGE BACKGROUD COLOR FOR BUTTON
		...
		tf1.setText(s[comboBox1.getSelectedIndex()]);		//SETTING THE VALUE OF COMBOBOX INTO TEXFIELD
		
		tf2.setBackground(Color.GRAY);
		tf2.setForeground(Color.white);
		tf2.setText(s[comboBox2.getSelectedIndex()]);
		
		tf3.setBackground(Color.GRAY);
		tf3.setForeground(Color.white);
		tf3.setText(s[comboBox3.getSelectedIndex()]);
		...
		f.add(panel);						//ADDING PANNELS TO FRAME
		f.add(panel1);				
		f.add(tf);								
		f.add(tf1);							//ADDING TEXTFIELDS TO FRAME
		f.add(tf2);
		f.add(tf3);
		f.add(b10);
		f.add(b3);

		f.setLayout(new GridLayout(4,3,90,90));
		f.addWindowListener(new WindowEventListener());
		f.pack();
		f.setBackground(Color.orange);
		f.setVisible(true);

		comboBox1.addItemListener(							//ACTION ON COMBOBOX1
			...
			new ItemListener(){
				public void itemStateChanged(ItemEvent e)
				{
					...
					{
						tf1.setText(" ");
						tf1.setText(s[comboBox1.getSelectedIndex()]);
					}
				}
		...
		comboBox2.addItemListener(						//ACTION ON COMBOBOX2
		...
		comboBox3.addItemListener(					//ACTION ON COMBOBOX3
	...
	{		
		...
		if(e.getSource()==b2)								//ACTION WHEN COLOR BUTTON IS CLICKED TO CHANGE BACKGROUND COLOR  
		{
		...
		else if(e.getSource()==b3)				//SEND BUTTON TO SEND THE MESSAGE
		{
	...
	public void run()							
	{
		...
		for(;;)
		{
			try
			{
				...
				b10.setText("NOT CONNECTED");		//TO RESET THE STATE OF CONNECTION
				b10.setBackground(Color.black);
				b10.setForeground(Color.white);
				...
				b10.setText("CONNECTED");			//TO SET THE STATE OF CONNECTION
				b10.setBackground(Color.green);
				b10.setForeground(Color.black);
				/*
				PrintStream ps=new PrintStream(soc.getOutputStream());
				BufferedReader cl=new BufferedReader(new InputStreamReader(soc.getInputStream()));
				fname=cl.readLine();
				...
				if(f1.exists())
				{	
					b1.setText("FILE FOUND.");		//SET BUTTON TEXT INTO FILE FOUND
					...
					tf.setEditable(false);
								...
								//IF FILE FOUND THEN ENTER 3 KEY ONE BY ONE			
					key1= JOptionPane.showInputDialog("Enter Key 1(Minimum 8 Characters):");
					tf11.setText(key1);
					
					key2= JOptionPane.showInputDialog("Enter Key 2(Minimum 8 Characters):");
					tf22.setText(key2);
					
					key3= JOptionPane.showInputDialog("Enter Key 3(Minimum 8 Characters):");
					tf33.setText(key3);
					...
					String fname1="C"+fname;
					FileWriter fw=new FileWriter(fname1);
					String str;
					byte[][] keys = {
					key1.getBytes(),key2.getBytes(),key3.getBytes()
					};
					while((str=br.readLine())!=null)	//SENDING THE FILE TO CLIENT
					{
						//String str1=encrypt(str,key1);
						byte[] enc = MyServer1.TripleDES_Encrypt(str.getBytes(), keys);
						String str1=new String(enc);
				...
				else
				{
					...
					b1.setText("FILE NOT FOUND.");	//SETTING THE TEXT OF BUTTON TO FILE NOT FOUND.
